...
export function SuggestedForYou() {
  ...
  const [lastLikedSerialized, setLastLikedSerialized] = useState<string>("")
  ...
  const buildRecommendations = useCallback(async () => {
    ...
    try {
      ...
      setLastLikedSerialized(JSON.stringify(Array.from(likedIds).sort()))
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Check if likes have changed since we last fetched
    const currentSerialized = JSON.stringify(Array.from(likedIds).sort())
    if (tracks.length > 0 && !isLoading && lastLikedSerialized && currentSerialized !== lastLikedSerialized) {
    ...
    buildRecommendations()
  }, [isLoaded, isSignedIn, isLoadingLikes, likedIds, tracks.length, isLoading, lastLikedSerialized, buildRecommendations])
