  ...
  async function handleRemove(trackId: number) {
    try {
      ...
      setPlaylist((prev: any) => ({
        ...prev,
        tracks: prev.tracks.filter((t: any) => t.trackId !== trackId)
      }))
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="flex items-center gap-4 mb-8">
        <button
          onClick={onBack}
          className="size-10 rounded-full border-0 bg-surface-elevated flex items-center justify-center shrink-0 text-muted hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
        >
